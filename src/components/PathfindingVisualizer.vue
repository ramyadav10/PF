<template>
  <div class="pathfinding-visualizer" @click="clearFocus">
    <VisualizerCanvas
      ...
      @switchControlType="
        controlType = controlType == 'Orbit' ? 'PointerLock' : 'Orbit'
      "
    ...
    >
      ...
      <span class="lg">{{
        worldSetup ? "Complete Setup" : "Setup World"
      }}</span>
    </Button>
    <Button
    ...
    >
      <img
        src="@/assets/icons/street-view.svg"
        alt=""
        v-if="controlType == 'Orbit'"
      />
      <img src="@/assets/icons/perspective.svg" alt="" v-else />
      <span class="lg">{{
        controlType == "Orbit" ? "First-person" : "Perspective"
      }}</span>
    </Button>
    <Button
    ...
    <Button
      class="hover btn-map warning"
      key="navigate-map"
      @click="navigateToMap"
      v-if="true"
    >
      <img src="@/assets/icons/map.png" alt="" />
      <span class="lg">Go to Map</span>
    </Button>
    <input
      id="threshold"
      ref="threshold"
...
import {
  ...
  tweenToColor,
} from "./algorithms/helpers.js";
import {
  randomMaze,
  recursiveDivisionMaze,
} from "./algorithms/mazeAlgorithms.js";
    ...
    deviceCamInput: function (newVal, oldVal) {
      if (newVal) {
        ...
        if (hasGetUserMedia()) {
          ...
          const constraints = {
            video: {
              width: { exact: width / scale },
              height: { exact: height / scale },
            },
          };
  ...
  methods: {
    ...
    moveCamera() {
      ...
      new TWEEN.Tween(this.$refs.visualizer.camera.position)
        ...
        .onUpdate(() => {
          this.$refs.visualizer.camera.lookAt(
            this.$refs.visualizer.scene.position
          );
